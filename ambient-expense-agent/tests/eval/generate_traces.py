...
async def generate_all_traces():
    ...
    for idx, case in enumerate(eval_cases):
                    ...
                    if event.content and event.content.parts:
                        for part in event.content.parts:
                            if getattr(part, "text", None):
                                ...
                                if getattr(
                                    fc, "name", ""
                                ) == "adk_request_input" and isinstance(fc.args, dict):
        ...
        if interrupt_msg:
            is_security_alert = (
                "CRITICAL SECURITY ALERT" in interrupt_msg
                or "PROMPT INJECTION" in interrupt_msg
            )
            decision = "reject" if is_security_alert else "approve"
            print(
                f"  [HITL Intercepted] Prompt Injection Alert={is_security_alert} -> Decision: '{decision}'"
            )
        ...
        output_cases.append(
            {
                ...
                "prompt": {"role": "user", "parts": [{"text": sanitized_prompt_text}]},
                "responses": [
                    {
                        "response": {
                            "role": "model",
                            "parts": [
                                {"text": final_text.strip() or "Workflow completed."}
                            ],
                        }
                    }
                ],
                "agent_data": {
                    ...
                    "turns": [{"turn_index": 0, "events": turns_events}],
                },
            }
        )
